...
def showCategorys():
    ...
    if 'username' not in login_session:
        return render_template('publicCategorys.html', Categorys=Categorys, items=items)  # noqa
    else:
...
def newCategoryItem(games_name):
    ...
    Categorys = session.query(Category).filter_by(name=games_name).one()
    if login_session['user_id'] != Categorys.user_id:
    ...
    if request.method == 'POST':
        if request.form['name']:
            newItem = CategoryItem(name=request.form['name'], description=request.form['description'], games_category_id=request.form['Category_id'], user_id=Categorys.user_id)  # noqa
            session.add(newItem)
            session.commit()
...
def editCategoryItem(games_category_name, games_item_name):  # noqa
    ...
    if login_session['user_id'] != Categorys.user_id:  # noqa
